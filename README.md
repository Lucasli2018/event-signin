...
### 活动广场（公开发现）
- **活动广场（首页 `/`）**：所有人无需登录即可浏览所有公开活动（未删除 / 未归档 / 且组织者未隐藏），支持关键词 / 状态 / 名额 / 排序筛选；点「＋ 创建活动」进入创建页。`/plaza.html` 保留为跳转首页的兼容别名。
...
```
public/                 静态前端
  index.html            活动广场（首页，公开浏览 + 条件搜索，无需登录）
  create.html           创建活动
  plaza.html            活动广场兼容别名（自动跳转首页 /）
